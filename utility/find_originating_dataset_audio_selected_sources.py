...
PROJECT_ROOT = SCRIPT_DIR.parent if SCRIPT_DIR.name == "utility" else SCRIPT_DIR
...
DEFAULT_SOURCE_FILES = [
    "000_000020.mp3","000_000026.mp3","000_000030.mp3","000_000046.mp3","000_000048.mp3","000_000185.mp3","000_000246.mp3","000_000248.mp3"
]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare a generated audio file against a selected set of dataset source files using tokenizer-code overlap and audio similarity."
    )
    parser.add_argument("input_audio", help="Generated/inference output audio file to analyze.")
    parser.add_argument(
        "sources",
        nargs="*",
        help="Optional source filenames to compare. If omitted, uses the built-in selected source list.",
    )
    parser.add_argument("--tokenizer-dir", default="latent_audio_tokenizer_out")
    parser.add_argument("--top-k", type=int, default=DEFAULT_TOP_K)
    parser.add_argument("--max-source-seconds", type=float, default=30.0, help="Trim source candidates before encoding codes.")
...
def select_items(all_items: List[Dict[str, str]], source_files: List[str]) -> List[Dict[str, str]]:
    wanted = {name.strip() for name in source_files if name.strip()}
    selected = [item for item in all_items if item["file"] in wanted]
    missing = sorted(wanted - {item["file"] for item in selected})
    if missing:
        print("Warning: the following requested source files were not found:")
        for name in missing:
            print(f"- {name}")
    return selected
...
def main():
    ...
    all_items = load_dataset_items(tokenizer_config.metadata_csv, tokenizer_config.audio_dir)
    if not all_items:
    ...
    items = select_items(all_items, source_files)
    if not items:
        raise RuntimeError("None of the selected source files were found in the dataset.")

    print(f"Input audio: {input_path}")
    print(f"Selected source candidates: {len(items)}")
    ...
    if not ranked:
        raise RuntimeError("No readable selected candidates could be compared.")

    best = ranked[0][1]
    print()
    print("Most likely originator among selected sources:")
    ...
    print()
    print(f"Top {min(args.top_k, len(ranked))} selected-source matches:")
