...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate prompt-conditioned audio with fixed-duration source themes, rank-relaxed token sampling, and energy gates for windows and clips, using broader and more varied source selection."
    )
    ...
    parser.add_argument("--theme-temperature", type=float, default=1.15, help="Higher values make source theme selection more varied within the top-N pool.")
...
def choose_theme_entry(
    ...
    scores = [max(0.05, float(entry.get("match_score", 0.0)) + 0.25) for entry in working]
...
def main():
    ...
    # Keep the generation path deterministic from the user seed, but let source selection vary more broadly.
    source_rng = random.Random()
    ...
    for clip_idx in range(clip_count):
        ...
        print(f"Generating varied source-creative latent clip {clip_idx + 1}/{clip_count} on {device}...")
        ...
        for retry_idx in range(retry_count):
            ...
            clip_args.theme_top_n = max(1, clip_args.theme_top_n)
    ...
    print(f"Saved varied source-creative-ranked latent audio to {output_path}")
