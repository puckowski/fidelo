...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate prompt-conditioned audio with fixed-duration source themes, rank-relaxed token sampling, and energy gates for windows and clips, with a sliding-window bias toward repeating the same source sample."
    )
...
def main():
    ...
    for clip_idx in range(clip_count):
        print(f"Generating sticky source-creative latent clip {clip_idx + 1}/{clip_count} on {device}...")
    ...
    print(f"Saved sticky source-creative-ranked latent audio to {output_path}")
